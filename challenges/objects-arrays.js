...
*/
const animalNames = [];
zooAnimals.forEach(an => {
  animalNames.push(
    `Name: ${an.animal_name}, Scientific: ${an.scientific_name}`
  );
});
...
const lowerCase = zooAnimals.map(an => an.animal_name.toLowerCase());
...
*/
const largerPopulation = zooAnimals.filter(an => an.population < 5);
